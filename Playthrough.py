...
class RewindablePlaythrough(Playthrough):
    ...
    def copy(self):
        new_cache = {k: copy.copy(v) for k,v in self._cache.items()}
        new_playthrough = RewindablePlaythrough(self.state_list, new_cache)
        new_playthrough.cached_spheres = [{k: copy.copy(v) for k, v in sphere.items()} for sphere in self.cached_spheres]
        new_playthrough._state_cache = [[state.copy() for state in state_list] for state_list in self._state_cache]
        new_playthrough.cache_level = self.cache_level
